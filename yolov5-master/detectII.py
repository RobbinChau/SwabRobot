    ...
    for i, det in enumerate(pred):  # per image 每张图片
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                xy0 = [(int(xyxy[0].item()) + int(xyxy[2].item())) / 2 ,
                ...
                cls = names[int(cls)]
                conf = float(conf)
                detections.append({'class': cls, 'conf': conf, 'position': xy0})
                GobalV.set_value('class',cls)
                GobalV.set_value('conf', conf)
                GobalV.set_value('position', xy0)


                # Stream results
                im0 = annotator.result()
                #if True:#view
                    #cv2.imshow(str(p), cv2.resize(im0, (640, 480)))

                # Save results (image with detections)
                if True:#save
                    if dataset.mode == 'image':
                        cv2.imwrite(save_path, im0)
                        cv2.imwrite('./yolov5-master/runs/Cap/'+'capturede'+'.jpg', im0)
...
path = './yolov5-master/runs/detect/exp33/capture.jpg'
#path = './yolov5-master/runs/Cap/capture.jpg'
